...
if [[ -f "createfiles.sh" && -f "createtestfiles.sh" ]]; then
    ...
    [[ ! -f "createfiles.sh" ]] && echo "Error: createfiles.sh not found in the current directory!"
    [[ ! -f "createtestfiles.sh" ]] && echo "Error: createtestfiles.sh not found in the current directory!"
    exit 1
fi
